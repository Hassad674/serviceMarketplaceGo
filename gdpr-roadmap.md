...
| B.2 | `audit_logs` cold-storage archiving (24mo → R2 jsonl.gz then DELETE) | 6h | ✅ | `backend/internal/adapter/r2/audit_archive.go`, `backend/internal/port/service/audit_archive_writer.go`, `backend/internal/adapter/postgres/retention_repository_r2.go`, `backend/internal/domain/retention/policy.go` (StrategyArchiveToR2), `backend/internal/domain/retention/policies.go` (`audit_logs_archive_to_r2_24mo` policy), `backend/cmd/api/wire_retention.go`, `backend/internal/config/config.go` (`AuditColdStorageBucket`), migration 149 (`audit_logs_archive.r2_key`), R2 path `audit-cold/<yyyy>/<mm>/<batch_id>.jsonl.gz` | `internal/adapter/r2/audit_archive_test.go` (6 unit tests, httptest fake S3, 117× compression observed), `internal/adapter/postgres/retention_repository_r2_test.go` (4 integration tests gated by `MARKETPLACE_TEST_DATABASE_URL`) | DONE — two-phase sweep: tick 1 SELECTs pending rows (`r2_key IS NULL`), uploads gzipped JSONL to R2, UPDATEs `r2_key`. Tick 2 hard-deletes rows that already carry an `r2_key`. Crash-safe at every boundary. Year/month bucketing in keys enables future prefix-range deletion. Feature gated by `STORAGE_AUDIT_COLD_BUCKET` — empty = sweep skips itself with a one-line WARN per tick. Cost rationale: R2 ~$0.015/GB/mo + zero egress vs Neon ~$0.10/GB/mo = ~85% savings on the cold tier. |
...
| B.4 (session) | Server-side cookie/session expiry tracking + cleanup (gdpr-audit.md Section 11 + "Refresh token rotation") | 4h | ✅ DONE | migration `147_create_user_sessions`, `backend/internal/domain/session/`, `backend/internal/port/repository/session.go`, `backend/internal/adapter/postgres/session_repository.go`, `backend/internal/app/auth/session_audit.go` + hooks in `service.go`/`service_more.go`, `backend/internal/handler/auth_handler_fingerprint.go`, retention `StrategyDeleteRevokedSessions` + `user_sessions_revoked_30d_delete` policy | `domain/session/entity_test.go` (3 tests), `app/auth/session_audit_test.go` (6 tests), `adapter/postgres/session_repository_test.go` (6 integration tests gated on MARKETPLACE_TEST_DATABASE_URL) | Persists JTI/parent_jti/UA hash/anon IP/login_method per refresh token rotation. Login + Register insert; Refresh revokes old + chains new via parent_jti; reuse-detection mass-revokes for the user. Retention scheduler purges revoked-and-stale rows after 30 days. Caveat: `invitation_handler.go` still issues refresh tokens directly (skipped per scope discipline) — flagged for a follow-up to share the auth.Service hook. |
