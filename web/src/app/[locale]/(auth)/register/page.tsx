        ...
        {roles.map((role) => {
          ...
          return (
            <Link
              key={role.id}
              href={role.href}
              className={`animate-scale-in group flex flex-col items-center rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-8 text-center shadow-sm transition-all duration-200 ${role.hoverBorder} ${role.hoverShadow} hover:shadow-lg`}
            >
              ...
              <div className="mt-5 flex items-center gap-1 text-sm font-medium text-gray-400 transition-colors group-hover:text-gray-900 dark:group-hover:text-white">
                {t("registerTitle")}
