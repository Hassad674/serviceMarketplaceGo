...
const agencySchema = z
  .object({
    display_name: z.string().min(2, "Le nom de l'agence est requis"),
...
export function AgencyRegisterForm() {
  ...
  async function onSubmit(values: AgencyValues) {
    setError(null)
    try {
    ...
    <div className="rounded-xl border border-border bg-card p-8 shadow-sm">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          />
          <p className="text-xs text-muted-foreground">Nom commercial de votre agence</p>
          {errors.display_name && (
        ...
        >
          {isSubmitting ? "Inscription..." : "Créer mon compte agence"}
        </button>
      </form>
