...
// ReferrerProfileCard is the compact directory tile for the
// /referrers listing page. Visually identical to the freelance card
// so the listing grids can mix persona types — the only giveaway is
// the commission pricing string in the footer.
export function ReferrerProfileCard({
  profile,
  displayName,
}: ReferrerProfileCardProps) {
  ...
  return (
    <article className="bg-card border border-border rounded-2xl p-5 shadow-sm transition-all duration-200 hover:shadow-md hover:border-rose-200 hover:-translate-y-0.5">
      <Link
        href={`/referrers/${profile.organization_id}`}
