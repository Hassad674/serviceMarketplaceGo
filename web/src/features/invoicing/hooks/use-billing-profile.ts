...
export function useBillingProfile() {
  return useQuery<BillingProfileSnapshot>({
    queryKey: invoicingQueryKey.profile(),
...
export function useUpdateBillingProfile() {
  ...
  return useMutation<BillingProfileSnapshot, Error, UpdateBillingProfileInput>({
    ...
    onSuccess: (snapshot) => {
      queryClient.setQueryData(invoicingQueryKey.profile(), snapshot)
      queryClient.invalidateQueries({ queryKey: invoicingQueryKey.profile() })
    },
  })
}
