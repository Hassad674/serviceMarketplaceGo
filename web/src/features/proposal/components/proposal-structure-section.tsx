...
import type { ProposalFormData, ProposalEscrowStructure, ProposalInvoiceBilling, ProposalInvoiceFrequency } from "../types"
import { ProposalMilestoneEditor } from "./proposal-milestone-editor"

type ProposalStructureSectionProps = {
  formData: ProposalFormData
  updateField: <K extends keyof ProposalFormData>(field: K, value: ProposalFormData[K]) => void
}

export function ProposalStructureSection({ formData, updateField }: ProposalStructureSectionProps) {
...
function EscrowStructureSection({ formData, updateField }: ProposalStructureSectionProps) {
    ...
    <section className="space-y-4">
      ...
      <div className="inline-flex rounded-lg bg-gray-100 dark:bg-gray-800 p-1">
        <ToggleButton
          ...
          onClick={() => updateField("escrowStructure", "milestone" as ProposalEscrowStructure)}
          label={t("milestone")}
        />
        <ToggleButton
          active={formData.escrowStructure === "one-time"}
          onClick={() => updateField("escrowStructure", "one-time" as ProposalEscrowStructure)}
          label={t("oneTime")}
        />
      </div>

      {formData.escrowStructure === "milestone" ? (
        <ProposalMilestoneEditor
...
function InvoiceStructureSection({ formData, updateField }: ProposalStructureSectionProps) {
  const t = useTranslations("proposal")
    ...
    <section className="space-y-4">
      ...
      <div className="inline-flex rounded-lg bg-gray-100 dark:bg-gray-800 p-1">
        <ToggleButton
          ...
          onClick={() => updateField("invoiceBillingType", "fixed" as ProposalInvoiceBilling)}
          label={t("fixed")}
        />
        <ToggleButton
          active={formData.invoiceBillingType === "hourly"}
          onClick={() => updateField("invoiceBillingType", "hourly" as ProposalInvoiceBilling)}
          label={t("hourly")}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateField("invoiceFrequency", e.target.value as ProposalInvoiceFrequency)}
            className={cn(
              "h-12 w-full rounded-xl border border-gray-200 dark:border-gray-700",
