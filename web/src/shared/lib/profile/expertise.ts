...
// domain key sent to the expertise endpoints must belong to this list.
// The i18n layer maps each key to a localized label.
//
...
// Narrows an unknown string into a known domain key — used to filter
// out unexpected values coming from the API (forward-compat with
// backends that may ship new keys before the frontend knows about them).
export function isExpertiseDomainKey(
  value: string,
): value is ExpertiseDomainKey {
...
export function getMaxExpertiseForOrgType(
...
export function orgTypeSupportsExpertise(
  orgType: string | undefined,
): boolean {
