...
func (r *JobRepository) Update(ctx context.Context, j *job.Job) error {
	...
	result, err := r.db.ExecContext(ctx, queryUpdateJob,
		j.ID, string(j.Status), j.ClosedAt, j.UpdatedAt,
	)
	if err != nil {
...
func (r *JobRepository) ListByCreator(ctx context.Context, creatorID uuid.UUID, cursorStr string, limit int) ([]*job.Job, string, error) {
	...
	if cursorStr == "" {
		...
		rows, err = r.db.QueryContext(ctx, queryListJobsByCreatorWithCursor,
			creatorID, c.CreatedAt, c.ID, limit+1)
	}
	if err != nil {
...
// scanJobFromRows scans a single job from a Rows iterator.
