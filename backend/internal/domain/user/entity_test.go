...
func TestUser_IsKYCBlocked(t *testing.T) {
	stripeID := "acct_123"
	...
	tests := []struct {
		...
		{
			name:     "no earnings — not blocked",
			user:     User{},
			expected: false,
		},
		{
			name:     "earnings within 14 days — not blocked",
			user:     User{KYCFirstEarningAt: &past5},
			expected: false,
		},
		{
			name:     "earnings over 14 days, no stripe — blocked",
			user:     User{KYCFirstEarningAt: &past15},
			expected: true,
		},
		{
			name:     "earnings over 14 days, stripe set — not blocked",
			user:     User{KYCFirstEarningAt: &past15, StripeAccountID: &stripeID},
			expected: false,
		},
		{
			name:     "no earnings, stripe set — not blocked",
			user:     User{StripeAccountID: &stripeID},
			expected: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expected, tt.user.IsKYCBlocked())
		})
	}
}

func TestUser_HasKYCCompleted(t *testing.T) {
	stripeID := "acct_123"
	emptyStripe := ""
	...
	tests := []struct {
		...
		{"no stripe account", User{}, false},
		{"empty stripe account", User{StripeAccountID: &emptyStripe}, false},
		{"valid stripe account", User{StripeAccountID: &stripeID}, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expected, tt.user.HasKYCCompleted())
		})
	}
}

func TestUser_KYCDaysRemaining(t *testing.T) {
	stripeID := "acct_123"
	past15 := time.Now().Add(-15 * 24 * time.Hour)
	past10 := time.Now().Add(-10 * 24 * time.Hour)
	...
	tests := []struct {
		...
		{"no earnings", User{}, -1},
		{"kyc completed", User{StripeAccountID: &stripeID}, -1},
		{"10 days elapsed — 3 remaining", User{KYCFirstEarningAt: &past10}, 3},
		{"15 days elapsed — 0 remaining", User{KYCFirstEarningAt: &past15}, 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expected, tt.user.KYCDaysRemaining())
		})
	}
}
